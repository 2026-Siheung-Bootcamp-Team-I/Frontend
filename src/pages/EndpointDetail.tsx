...
function Summary({ info }: { info: Host }) {
  ...
  return (
    <>
      ...
      {/*
        상태와 위험도는 기준이 달라 서로 어긋나 보일 수 있다. 상태는 가장 높은 심각도 하나로 판정하고
        위험도는 열린 알림을 전부 더한다. 보통 알림이 수십 건 쌓이면 상태는 정상인데 위험도만 100 이 된다.
        몇 점부터 위험인지는 서버가 정하지 않았으므로 화면이 임계값을 지어내지 않고 차이를 설명한다.
      */}
      <div className="mt-[14px] text-[12px] leading-[1.5] text-faint">
        상태는 가장 높은 심각도 하나로 정해지고, 위험도는 열린 알림을 심각도로 가중해 모두 더한
        0~100 값입니다. 그래서 낮은 심각도가 많이 쌓이면 상태는 정상이어도 위험도가 높게 나옵니다.
        둘이 어긋나면 아래 목록에서 실제 알림을 확인하세요.
      </div>
    </>
  )
}
